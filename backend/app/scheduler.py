...
class Scheduler:
    ...
    async def _execute_job(self, job_id: str, cfg: dict[str, Any], *, one_time: bool = False) -> None:
        ...
        try:
            if is_script:
                # `script:`-type cron jobs run a plain Python entrypoint (the v4
                # image has no `uv`); FREN_* context exported via env.
                ...
                # Wrap the task prompt with a scheduled-trigger header when
                # targeting chat/persona agents — they must not mistake a
