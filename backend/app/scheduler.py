...
Faithful v4 port of v3 ``fren/telegram/scheduler.py``. Reads recurring jobs from
``config/schedule.yml`` and one-time / limited-run jobs from
``config/one_time_schedule.yml``, evaluates cron due-ness with croniter, and
fires the configured agent (or ``script:`` tool) as a subprocess. Tracks
per-job last-run state in ``data/scheduler_state.json``, applies a circuit
breaker on repeated failures, reconciles stale DB executions, and shuts down
cleanly via :meth:`Scheduler.stop`.

Cron evaluation, due-job selection, the agent-spawn call, the circuit breaker,
and shutdown handling are preserved exactly from v3.
"""
...
class Scheduler:
    ...
    async def _execute_job(self, job_id: str, cfg: dict[str, Any], *, one_time: bool = False) -> None:
        ...
        if is_script:
            ...
            # TODO(v4-port): verify agent-spawn wiring. v3 shelled out to
            # ``uv run scripts/opencode_manager.py run --agent <agent> ...`` from the
            # project root. The v4 worker layer is app.runtime.runner.run_agent, which
            # takes (agent_dir, agent_name, prompt, backend, timeout_s) and resolves
            # compiled .opencode trees under get_settings().agents_dir. The subprocess
            # invocation below is preserved verbatim from v3; switching it to
            # run_agent() (see _run_via_runner) is the intended v4 wiring but is left
            # opt-in until the opencode_manager-equivalent run-logging is confirmed.
            cmd = [
                "uv",
                "run",
    ...
    # via the v4 worker layer (app.runtime.runner.run_agent). Not used by the
    # default _execute_job above (which preserves v3's subprocess shell-out), but
    # provided so the agent-spawn can be migrated to the compiled-agents tree
    # under get_settings().agents_dir once run-logging parity is confirmed.
    # ------------------------------------------------------------------
    @staticmethod
    async def _run_via_runner(agent: str, prompt: str, *, timeout: int = 600) -> int:
        from app.runtime.runner import run_agent  # TODO(v4-port): verify agent-spawn wiring

        agents_dir = Path(get_settings().agents_dir)
